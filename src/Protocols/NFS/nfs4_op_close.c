		  ...
		  struct nfs_resop4 *resp)
{
	...
	PTHREAD_RWLOCK_wrlock(&state_obj->state_hdl->state_lock); //done_stchecked
	...
	glist_for_each(glist, &state_found->state_data.share.share_lockstates) {
		...
		if (!glist_empty(&lock_state->state_data.lock.state_locklist)) {
			...
			PTHREAD_RWLOCK_unlock(
				&state_obj->state_hdl->state_lock); //done_stchecked
			LogDebug(COMPONENT_STATE,
	...
	PTHREAD_RWLOCK_unlock(&state_obj->state_hdl->state_lock); //done_stchecked
	res_CLOSE4->status = NFS4_OK;
